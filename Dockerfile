...
WORKDIR /root
RUN /bin/bash -c "apt update && apt upgrade -y && apt install curl git build-essential libssl-dev pkg-config -y"
...
RUN /bin/bash -c "source /root/.cargo/env && rm -rf /root/iodine_storage && rustup self uninstall -y && apt purge curl git build-essential libssl-dev pkg-config -y && apt autoremove -y"
